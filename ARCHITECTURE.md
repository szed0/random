...
flowchart TB
    ...
    packer["Evidence-pack builder"]
    gemini["Gemini web Gems<br/>read the patents, outside world,<br/>white space"]
    inbox["Insight inbox<br/>checks every cited patent number"]
    folder["Project folder<br/>technical_terms.csv, secondary CSV / JSON,<br/>packs, insights, report"]

    exportfile --> engine --> analytics --> gui
    gui -- "Send to Gemini" --> packer
    packer -- "clipboard" --> gemini
    gemini -- "analyst pastes answer" --> inbox
    inbox --> gui
    engine --> folder
    analytics --> folder
    packer --> folder
