...
This is the v3 reference. It describes what the code does, why each choice was
...
- [8. Stage 5 — relations](#8-stage-5--relations)
...
- [12b. Domain specificity](#12b-domain-specificity)
...
```
  ...
  keep = set(variants)         canonical names only
      │
      ├─ cooccurrence()      -> edges     [source, target, documents, ppmi]
      ├─ typed_relations()   -> relations [source, relation, target, documents]
      ├─ domain_specificity()-> spec      [domain, term, documents, log-odds, z]
      └─ trends() + trajectory() -> [term, documents, growth, state, <period>...]
      ▼
  Result{corpus, terms, edges, relations, specificity, trends, canonical,
         variants, per_doc, doc_canon, notes}
```
...
`notes` is the diagnostic channel. When a result looks thin, the reason travels
with it instead of being lost. The UI renders each note as an info banner.
...
The lookarounds are the whole point. A bare `(19|20)\d{2}` matches inside
`US2024123456A1` and yields 2024 — a fabricated year in a chart that looks
entirely plausible. Requiring non-digits on both sides rejects it.

`_year` dispatches on type before falling back to the regex:

- `datetime` / `date` / `Timestamp` → `.year`
- numeric in `[20000, 80000]` → Excel serial, converted from the 1899-12-30
  epoch. pandas leaves these as raw integers whenever the column has any
  non-date cell, which is common in real exports.
- numeric in `[1600, 2199]` → a bare year
- anything else → the bounded regex, else `None`

`date_warning()` then reports, without failing, when a chosen column yields no
years at all or fewer than half. An empty trend tab and a fabricated one look
identical to a user; this is the only thing that distinguishes them.
...
The parser cannot be disabled — `noun_chunks` raises `E029` without it, which
`run()` checks for explicitly rather than letting it surface mid-loop. The
tagger cannot go either, since the lemmatiser depends on it and the participle
rule below reads `tag_`.
...
`nlp.pipe` is fed batches bounded by **total characters**, not document count.
The parser allocates per character, so a fixed `batch_size=48` is free for short
abstracts and fatal for long ones. Measured, 48 documents of 100,000 characters:

| Batching | Result |
|---|---|
| `batch_size=48` (v1) | `MemoryError`: unable to allocate 1023 MiB |
| `batch_size=8`, same input | completes, +34 MB peak |
| character-bounded (v2, full pipeline) | completes, 49.0s |

What this buys, measured against v1 on a byte-identical corpus: the crash goes
away, and peak RSS at 40,000 rows drops from 70 MB to 39 MB. **Throughput is
unchanged** — 55.8s vs 57.9s, which is noise. The fix caps how much the parser
can be asked for at once; it does not make parsing faster.

Documents longer than `nlp.max_length` are truncated with a note rather than
raising `E088`. Abstracts never approach it; a description column mapped here by
mistake does.
...
```python
ceiling = max(int(cfg.max_doc_ratio * n_docs), cfg.min_term_freq)
```

The `max(...)` clamp is load-bearing. Without it the ceiling can fall below
`min_term_freq` and the two filters cross, making the admissible band empty:

| Documents | floor | raw ceiling | admissible |
|---|---|---|---|
| 1–3 | 2 | 0–1 | **impossible** |
| 4 | 2 | 2 | 2..2 |
| 10 | 2 | 6 | 2..6 |

And if *every* candidate is above the ceiling, the ceiling is skipped entirely
rather than returning nothing — a narrow corpus's shared vocabulary is its
subject, not its noise. `run()` records that in `notes`.
...
CSLS, mpnet and two cross-encoders were all tested and none fixes it.

The shipped 0.75 floor sat **above** two true pairs, so `leak detection sensor`
and `hydrogen leak detection sensor` were reported as two technologies with the
document count split between them.

What did work was adding exact rules for the relations the embedding cannot
represent at all — initialism and spelling convention — which took accuracy from
0.792 to 0.909 and abbreviations from 0/8 to 7/8. Those run as union passes
before the k-NN pass, over every pair rather than only near ones, because `MEA`
and `membrane electrode assembly` sit at cosine 0.013.
...
Two kinds, from two different signals. PPMI says which technologies **appear
together** more than chance; typed relations say **how** a sentence connected
them. trt-pb reported the second and not the first; techscope reports both.

### Typed relations, from the dependency parse

trt-pb found each adposition and walked left and right until it hit another
adposition, taking whatever raw tokens lay between as the two ends. That picks up
determiners, verb fragments and pieces of the neighbouring clause, and it cannot
tell which noun the preposition actually attaches to.

The parse is already being computed for noun chunks, so the correct arcs are
free:

| Shape | Source | Target |
|---|---|---|
| `prep` on a noun | the `prep` token's head | its `pobj` child |
| `prep` on a verb — "the tank **is coupled to** a bed" | the verb's `nsubj` | its `pobj` child |
| linking verb — "a tank **comprising** a jacket" | the noun it modifies | its `dobj` |

`_terms_with_spans()` records which token indices each term was built from, so a
token the parser attached a preposition to can be mapped back to the term
containing it. That map is the whole mechanism.

The preposition-to-class table is trt-pb's own, kept verbatim so a reader of the
original recognises the output — Inclusion, Objective, Effect, Process, Likeness,
and Misc for anything unlisted. The classes are the product's vocabulary, not a
linguistic claim.

The linking-verb case is new. trt-pb listed `includes` and `utilizes` among its
Inclusion triggers but scanned only adpositions, so it could never match them.
Measured on a 360-row corpus whose every abstract says "X is coupled to Y":
**25 relations with 0 Effect** before the verb cases were added, **118 with 92
Effect** after.

Counted by distinct documents, not occurrences: a patent repeating a phrase eight
times is one piece of evidence, the same choice made for term frequency
everywhere else.

### Co-occurrence by PPMI
...
Both versions produce identical term and edge counts, which is the point — the
v2 changes are correctness and failure handling, not different analysis. Runtime
is the same within measurement noise. The one real difference is peak memory at
scale, from bounding the parser's batch size.

An earlier draft of this document claimed the batching change roughly halved
runtime. It does not. That figure came from comparing two runs whose corpus
generators differed in abstract length, so it measured the corpora rather than
the code.
...
## 12b. Domain specificity

Only when the export carries a classification column — CPC, IPC, or any
domain-shaped field. ORBIT exports several per patent, newline separated, so each
record holds a list.

The question is what a domain does that the rest of the corpus does not. The
naive answer is a rate ratio, `P(term|domain) / P(term)`, which is what trt-pb
used and which puts a term seen twice in a small domain at the top of the chart
with a lift of 40.
...
```
delta = log[ (y_d + a) / (n_d + a₀ − y_d − a) ] − log[ (y_r + a) / (n_r + a₀ − y_r − a) ]
var   = 1/(y_d + a) + 1/(y_r + a)
z     = delta / sqrt(var)
```

`a` is the term's corpus-wide share of the prior mass `a₀`. The prior pulls a
rare term back toward the corpus rate in proportion to how little evidence
supports it; the z-score says how much of the remaining difference is real. A
frequency floor drops terms below `domain_floor` documents before any of this.

## 12c. Trajectory states

The trend table gave a slope. A slope is not a reading.

| | rising | flat or falling |
|---|---|---|
| **at or above median documents** | Growing | Mature |
| **below median** | Emerging | Fading / Niche |

"Established" is the corpus median rather than a fixed count, so the reading
means the same thing on 200 patents and on 40,000. A term below `min_docs` is
called **Sparse** rather than assigned a quadrant — the slope of three documents
is noise, and naming it Emerging would be an invention.

## 13. What changed

Same algorithms throughout. The changes are correctness and failure handling.

| Area | v1 | v2 |
|---|---|---|
| Year regex | `(19\|20)\d{2}` anywhere | boundary-anchored, 1600–2199, plus datetimes and Excel serials |
| `US2024123456A1` as a date | silently → 2024 | rejected; UI names the column and the value |
| Frequency floor vs ceiling | could cross; ≤3 documents always returned nothing | ceiling clamped to ≥ floor |
| Narrow corpus | ceiling erased every term | ceiling skipped, reason recorded in `notes` |
| Non-English input | empty report, no signal | `latin_share()` reports it |
| title == abstract column | accepted, text counted twice | rejected in `check()` |
| Batching | `batch_size=48` → `MemoryError` on long text | character-bounded; crash gone, peak RSS at 40k rows 70 MB → 39 MB, runtime unchanged |
| Oversize document | `E088` | truncated with a note |
| Missing / parser-less model | raw traceback mid-run | `InputError` with the install command |
| Diagnostics | none | `Result.notes`, rendered as banners |
| Streamlit cache | DataFrame re-hashed every rerun | `_frame` underscore-prefixed; digest is the whole key |
| OpenMP guard | `run_local.py` only | also `app.py`, covering `streamlit run` |
| First-ever run | hung on Streamlit's `Email:` prompt | launcher writes the empty credentials file |
...
- **All correctness testing used synthetic corpora.** The failure modes in §13
  were found by construction. Real exports will surface things these did not.
