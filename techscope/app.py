...
RELATION_HELP = {
    "Inclusion": "one is part of, or is used within, the other",
    "Objective": "the first exists for the purpose of the second",
    "Effect": "the first acts on or produces the second",
    "Process": "the second happens through or during the first",
    "Likeness": "the two are presented as comparable",
    "Misc": "linked by a preposition outside the five classes",
}


def view_relations(r: P.Result) -> None:
    """Typed technology relationships — the shape trt-pb reported."""
    if r.relations.empty:
        st.info("No typed relationship appears in enough documents. These come "
                "from grammatical links between two extracted technologies, so "
                "they need both ends to be terms that survived the filters.")
        return

    st.caption(
        "Read from the dependency parse: a preposition's governing noun and its "
        "object, or a linking verb and its object. Counted by **documents**, so "
        "one patent repeating a phrase is one piece of evidence. The five "
        "classes are trt-pb's own.")

    counts = r.relations["relation"].value_counts()
    cols = st.columns(len(counts))
    for col, (kind, n) in zip(cols, counts.items()):
        col.metric(kind, f"{n:,}", help=RELATION_HELP.get(kind, ""))

    kinds = ["all"] + list(counts.index)
    pick = st.radio("Relationship class", kinds, horizontal=True)
    shown = r.relations if pick == "all" else r.relations[r.relations["relation"] == pick]
    if pick != "all":
        st.caption(f"**{pick}** — {RELATION_HELP.get(pick, '')}")

    st.dataframe(
        shown.rename(columns={"source": "From", "relation": "Relationship",
                              "target": "To", "documents": "Documents"}),
        use_container_width=True, hide_index=True)
    st.download_button("Download relationships as CSV",
                       r.relations.to_csv(index=False).encode(),
                       "techscope-relationships.csv", "text/csv")


def view_domains(r: P.Result) -> None:
    """Domain specificity: what each domain does that the corpus does not."""
    if r.specificity.empty:
        st.info("Map a **Technology domain** column in the sidebar to see what "
                "distinguishes each domain. A CPC or IPC classification column "
                "is the usual one.")
        return
    ...
    st.caption(
        ...
        "& Quinn 2008). A plain rate ratio puts a term seen twice in a small "
        "domain at the top with a lift of 40; the prior pulls a rare term back "
        "toward the corpus rate in proportion to how little evidence supports "
        "it, and **z** says how much of the difference survives that.")

    domains = sorted(r.specificity["domain"].unique())
    pick = st.selectbox("Domain", domains)
    t = r.specificity[r.specificity["domain"] == pick].head(15)

    fig = px.bar(t.sort_values("z"), x="z", y="term", orientation="h",
                 hover_data=["documents", "corpus documents", "log-odds"],
                 labels={"z": "weighted log-odds z", "term": ""})
    fig.update_layout(height=max(300, 26 * len(t) + 120),
                      margin=dict(l=0, r=0, t=10, b=0))
    st.plotly_chart(fig, use_container_width=True)
    st.dataframe(t.rename(columns={"term": "Term", "documents": "In domain",
                                   "corpus documents": "In corpus"}),
                 use_container_width=True, hide_index=True)
...
def view_trends(r: P.Result) -> None:
    ...
    periods = [c for c in t.columns
               if c not in ("term", "documents", "growth", "recent share", "state")]

    if "state" in t.columns:
        st.caption(
            "Each technology is given a state rather than a bare slope. "
            "**Growing** and **Mature** are established terms rising or not; "
            "**Emerging** and **Fading** are smaller ones moving; **Sparse** "
            "means too few documents to call a direction, which is a reading in "
            "itself rather than a quadrant assignment.")
        counts = t["state"].value_counts()
        for col, (name, n) in zip(st.columns(len(counts)), counts.items()):
            col.metric(name, f"{n:,}")
...
def main() -> None:
    ...
    domain_guess = guess(cols, "domain", "cpc", "ipc", "classification", default="")
    domain_col = st.sidebar.selectbox(
        "Technology domain (optional)", ["— none —"] + cols,
        index=(cols.index(domain_guess) + 1) if domain_guess else 0,
        help="A classification column such as CPC. Enables domain specificity: "
    ...
    cfg = P.Config(
        ...
        domain_col="" if domain_col == "— none —" else domain_col,
    ...
    tabs = st.tabs(["Landscape", "Relationships", "Network", "Trends",
                    "Domains", "Explore"])
    with tabs[0]:
        view_landscape(result)
    with tabs[1]:
        view_relations(result)
    with tabs[2]:
        view_graph(result)
    with tabs[3]:
        view_trends(result)
