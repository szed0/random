...
def candidates(corpus: Corpus, nlp, cfg: Config,
               progress: Progress = _noop) -> tuple[list[list[str]], Counter]:
    """Normalised noun-chunk candidates per document, plus their document counts."""
    per_doc: list[list[str]] = []
    total = len(corpus)
    for n, doc in enumerate(nlp.pipe(corpus.docs, batch_size=48), start=1):
        seen: dict[str, None] = {}
...
class Result:
    ...
    def documents_for(self, term: str, limit: int = 20) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            "abstract": [self.corpus.abstracts[i][:240] + "…" for i in hits[:limit]],
        })
...
def run(path_or_buf, cfg: Config | None = None, *,
    ...
    per_doc, df = candidates(corpus, nlp, cfg, progress)
