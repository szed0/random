...
def analyse(frame: pd.DataFrame, _cfg: P.Config, cache_key: str) -> P.Result:
    """`cache_key` is a cheap digest of the frame and settings; Streamlit hashes
    that string rather than the DataFrame."""
    ...
    try:
        return P.run(None, _cfg, frame=frame,
...
def main() -> None:
    ...
    try:
        ...
        st.error("; ".join(exc.problems))
        return

    if result.terms.empty:
        st.warning("No terms survived. Lower **Minimum documents per term**, or "
                   "shorten the minimum term length.")
        return
