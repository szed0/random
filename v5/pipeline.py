...
def canonicalise(terms: Sequence[str], vectors: np.ndarray, df: Counter,
    ...
    for members in groups.values():
        ...
        # An acronym is a worse label than the words it stands for, whatever
        # the counts say, so a one-word member of a group that has a multiword
        # one loses. Otherwise: least boilerplate, then most documents.
        multiword = any(len(terms[i].split()) > 1 for i in members)
        head = max(members, key=lambda i: (
            len(terms[i].split()) > 1 if multiword else True,
            -_filler(terms[i]), df[terms[i]], -len(terms[i])))
        name = terms[head]
